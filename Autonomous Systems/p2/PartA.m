...
%Implementation of part A by Steven Ly z5257127
...
% ---------------------------------------------------------------------------------
function Solution_Aonly(filename)
%filename is string
...
for i=1:ne,
    ...
    [prior,covar] = get_pose(X,vw,dt,p_k); %part A
%     plot_ugv(ph,prior_bias)
    plot_ugv(ph,prior);%simulate ugv
    X = prior;  
    p_k = covar;
    ...
    switch sensorID    %measurement is from?
        ...
        [landmarks,ooi_local] = data_assoc(pp,ph,X,etc,ooi,Landmarks); %data association
        [~,cols] = size(landmarks);

        if cols > 0
            for j = 1:cols
           
                [X,ekf_var]  = updatestep(p_k,prior,[ooi_local(1,j);ooi_local(2,j)],[landmarks(1,j);landmarks(2,j)]);
                p_k = ekf_var;
                prior = X;
...
function [est_pos,p_cov] = get_pose(curr_pose,vw,dt,p_k) %part A
    ...
    %State Prediction
    dpose = [vw(1)*cos(curr_pose(3)); vw(1)*sin(curr_pose(3)); vw(2)];
    est_pos = curr_pose + dt*dpose;
    
    j_k = [1,0,-dt*vw(1)*sin(curr_pose(3));0,1,dt*vw(1)*cos(curr_pose(3));0,0,1];
%     p_u = 0.6*[(5/100)^2,0;0,(pi/180)^2];
    p_u = [(5/100)^2,0;0,(pi/180)^2];

    f_k = [dt*cos(curr_pose(3)),0;dt*sin(curr_pose(3)),0;0,dt];
    p_cov = j_k*p_k*j_k' + f_k*p_u*f_k';
...
function [ekf_pose,ekf_var] = updatestep(p_k,est_pose,ooi_measured,landmark)
   ...
   H_k_wrt =  [-(landmark(1)-xs)/(sqrt((landmark(1)-xs)^2 +(landmark(2)-ys)^2)),-(landmark(2)-ys)/(sqrt((landmark(1)-xs)^2 +(landmark(2)-ys)^2)),0;
   
   (landmark(2)-ys)/((landmark(1)-xs)^2 +(landmark(2)-ys)^2),-(landmark(1)-xs)/((landmark(1)-xs)^2 +(landmark(2)-ys)^2),-1
   ];
